...
df=pd.read_csv( read_path+'Thursday-WorkingHours-Morning-WebAttacks.csv' , low_memory = False , encoding = "cp1258" );
...
df.replace({' Label': 'Web Attack – Brute Force'},1,inplace=True)
df.replace({' Label': 'Web Attack – XSS'},2,inplace=True)
df.replace({' Label': 'Web Attack – Sql Injection'},3,inplace=True)
...
export_csv = df.to_csv (write_path+'cleadned_webAttack.csv', index = None, columns=write_columns,header=True, mode = 'w', encoding='cp1258')
