...
#DoS slowloris,,,,'DoS Slowhttptest,,,,DoS Hulk,,,Heartbleed,,,,DoS GoldenEye

read_path  = 'F:/Minor/cleaning/Uncleaned_Data/';
write_path =r'F:/Minor/New folder/'

df=pd.read_csv( read_path+'Wednesday-workingHours.csv' , low_memory = False , encoding = "cp1258" );
...
df.replace({' Label': 'DoS GoldenEye'},6,inplace=True)
df.replace({' Label': 'DoS Hulk'},7,inplace=True)
df.replace({' Label': 'DoS Slowhttptest'},8,inplace=True)
df.replace({' Label': 'DoS slowloris'},9,inplace=True)
df.replace({' Label': 'Heartbleed'},11,inplace=True)
...
# instead of using the LabelEncoder use OneHotEncoder 

# enc = OneHotEncoder(handle_unknown='ignore')
# enc.fit(df)

# saving the dataframe to new csv
export_csv = df.to_csv (write_path+'cleadned_restAll_Attack.csv', index = None, columns=write_columns,header=True, mode = 'w', encoding='cp1258')
