...
df=pd.read_csv( read_path+'Tuesday-WorkingHours.csv' , low_memory = False , encoding = "cp1258" );
...
df.replace({' Label': 'FTP-Patator'},10,inplace=True)
df.replace({' Label': 'SSH-Patator'},14,inplace=True)
...
export_csv = df.to_csv (write_path+'cleadned_patator.csv', index = None, columns=write_columns,header=True, mode = 'w', encoding='cp1258')
